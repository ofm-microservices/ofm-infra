...
if [[ -z "${gig_id}" || -z "${package_id}" ]]; then
  load_gig_defaults_from_log "$GIG_FLOW_LOG_FILE"
fi

if [[ -z "${gig_id}" || -z "${package_id}" ]]; then
  echo "ORDER_GIG_ID and ORDER_PACKAGE_ID are required, or run gig-flow first so order-flow can reuse gig-log.txt" >&2
  exit 1
fi
...
generate_jwt() {
  ...
  payload="$(
    jq -nc \
      ...
      --arg username "${ORDER_JWT_USERNAME:-order-flow}" \
      --argjson iat "$now" \
      --argjson exp "$exp" \
      '{
        sub: $sub,
...
access_token="${ORDER_ACCESS_TOKEN:-$(generate_jwt)}"
...
log "POST ${API_BASE_URL}/orders/start"
log "$start_payload"
start_status="$(
  request_json POST "${API_BASE_URL}/orders/start" "$start_payload" "$start_body"
)"
log "HTTP ${start_status}"
log_json_file "$start_body"

if [[ "$start_status" != "201" && "$start_status" != "200" ]]; then
  echo "start order failed" >&2
  exit 1
fi

order_id="$(jq -r '.order_id // .orderId // empty' "$start_body")"
...
log "$confirm_payload"
confirm_status="$(
  request_json POST "${API_BASE_URL}/orders/${order_id}/confirm" "$confirm_payload" "$confirm_body"
)"
log "HTTP ${confirm_status}"
log_json_file "$confirm_body"

if [[ "$confirm_status" != "200" ]]; then
  echo "confirm order failed" >&2
  exit 1
fi

checkout_url="$(
  jq -r '.checkout_url // .checkoutUrl // empty' "$confirm_body"
)"
if [[ -z "$checkout_url" || "$checkout_url" == "null" ]]; then
  echo "failed to extract checkout url" >&2
  cat "$confirm_body" >&2
  exit 1
fi
...
log "$checkout_url"
log_blank
log "Complete the payment in your browser."
log "Waiting for realtime payment event..."

if [[ -z "${realtime_listener_pid}" ]]; then
  start_realtime_listener
fi

while true; do
  if [[ -s "$realtime_event_file" ]]; then
    payment_event="$(cat "$realtime_event_file")"
    log "Realtime payment event: $payment_event"
    break
  fi
  if ! kill -0 "$realtime_listener_pid" >/dev/null 2>&1; then
    log "realtime listener exited before payment event"
    exit 1
  fi
  sleep 1
done
...
log_blank
log "== Done =="
log "Order flow completed through checkout handoff."
